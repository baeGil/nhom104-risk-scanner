...
- [ ] T6.1: **Set up Neo4j self-hosted** — Install Neo4j Community/Apocalypse, configure for 900K+ nodes. Set memory heap (≥4GB), page cache (≥2GB). Configure vector index plugin. — *Spec: segmentation*
...
- [ ] T6.3: **Build pipeline orchestration** — Python scripts for each phase. Idempotent (can resume from failure). Logging, progress tracking, error reporting. Configuration for batch sizes, Neo4j connection, embedding service URL. — *Spec: segmentation*
